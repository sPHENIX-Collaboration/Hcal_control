...
def lv_readi(tn,slot,channel, sector):
...
# get rid of blanks if they're there
    try:
        ...
        pass
    currents=[i for i in istring]
    NS=["N", "S"]
    currents_with_sector=["Sector "+sector+NS[i%2]+currents[i]+"\n" for i in range(len(currents))]
...
with open('power_currents_%s.txt' %inorout,'w') as f: 
    while True:
        ...
        if sector != "off" and sector !="OFF" and sector !="Off":
            ...
            tn.write(command.encode('ascii')+b"\n\r")
            tn.read_until(b'>')
        #current = lv_readi(tn,sl,ch, str(sector))
        #for c in current:
        #    f.write(c)
