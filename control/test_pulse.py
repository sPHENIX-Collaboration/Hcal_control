...
print("This script turns on/off the test pulse \nto all sectors using the $Pcbb command!")

if len(sys.argv) == 3:
    print("Usage: test_pulse.py {outer|inner|both} {east|west|all} {on|off}")
    sys.exit(0)
...
for h in hosts:
    print("TP "+arg_status+" for "+host_prefix+h)
    try:
    ...
    for c in range(8):
        # Pulse Mask
        # print("Sending: "+command_string+str(c)+setting_string)
        tn.write(bytes(command_string+str(c)+setting_string,encoding="ascii"))
