...
# print("Remember to set the gain on all  \nchannels to 'NORMAL' if using all LEDs! \n('HIGH' if only one.)")

if len(sys.argv) == 3:
    print("Usage: test_pulse.py {outer|inner|both} {east|west|all} {on|0|1|2|3|4|all|off}")
    sys.exit(0)
...
if(arg_detector=="both" and arg_half=="all"):
    ...
    print("please select ./led.y [outer|inner|both] [east|west|all] [on|0|1|2|3|4|all|off]")
    sys.exit(1)
...
mask_command = "$LM"
if(arg_status=="on" or arg_status=="all"):
    mask_setting = "001F"
elif(arg_status=="0"):
    mask_setting = "0001"
elif(arg_status=="1"):
    ...
    print("LED mask error! Please use only 'on,' '0,' '1,' '2,' '3,' '4,' 'all,' or 'off.'")
    sys.exit(2)
...
for h in hosts:
    ...
    print("Tiles "+mask_setting+" at Intensity "+dac_setting+" for "+host_prefix+h)
    if(arg_status=="off"):
        nleds = 5
    elif(int(h) < 90):
        nleds = 4
    ...
    for c in range(8):
        ...
        for l in range(nleds):
            tn.write(bytes(dac_command+str(c)+str(l)+dac_setting,encoding="ascii"))
            gg= tn.read_until(b">") # this command has no output, but requires a new line/carriage return
            #print(gg) # hangs script! :-(
            tn.write(b"\n\r")
            tn.write(b"\n\r")
            tn.write(b"\n\r")
        # LED Mask
        ...
        ggg= tn.read_until(b">") # do not comment out: command has text output
        #print(ggg)
        tn.write(b"\n\r")
        tn.write(b"\n\r")
        tn.write(b"\n\r")
